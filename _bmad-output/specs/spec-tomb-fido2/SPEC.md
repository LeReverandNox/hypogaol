...
- Device-backed create is inherently higher-risk than file-backed (the wrong device vs. a typo'd file path) — the tool must show an explicit warning describing the impending wipe and data loss and require the user's explicit confirmation before formatting any device target, even when no existing LUKS2 header was detected — **except** when the existing LUKS2 header carries CAP-23's hypogaol-owned marker token (the same exception line 134 above already carves out): a surviving marker is itself structural proof nothing of value occupies that device, so re-confirming would only reintroduce the manual-cleanup friction CAP-23 exists to remove; that case proceeds without the confirmation prompt.
  - **Addendum (2026-08-08):** exception clause added during Epic 6 architecture work to keep this constraint consistent with line 134's own CAP-23 exception, which already covered the sibling refuse-constraint but not this confirmation one. See the architecture spine's AD-9 and `.memlog.md` for the reasoning.
