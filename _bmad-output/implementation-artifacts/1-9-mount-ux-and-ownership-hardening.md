...
- [x] Task 2: Fix mount-point ownership — the actual bug (AC: #1)
  - [x] In `FilesystemBackend::mount`'s real implementation (`src/adapters/exec/mod.rs:896-955`), after the `mount` call succeeds and **before** the existing `chmod 0700` call, add `privileged("chown").arg(format!("{uid}:{gid}", ...)).arg(&mountpoint)` (both need root, since the mounted directory's underlying inode — created by a privileged `mkfs.ext4` at `create` time — is currently root-owned; a bare unprivileged `chown` would fail with EPERM)
  - [x] Keep the existing `chmod 0700` call afterward, unchanged — the combination (owned by the invoking user, `0700`) is what actually achieves "only the invoking user can access this," which the current `chmod`-only fix (root-owned, `0700`) does not
  - [x] On `chown` failure, follow the exact same cleanup pattern the existing `chmod` failure branch already uses (unmount, remove the directory, return `AdapterFailure`) — don't invent a different cleanup shape for the new failure branch
- [x] Task 3: Relocate the mount point from `/tmp` to `/run/media/<username>/<tomb-name>` (AC: #2)
  ...
  - [x] Base directory: `/run/media/<username>` (from Task 1's identity lookup). If it doesn't exist yet, create it via `privileged("mkdir").args(["-p"]).arg(&base)` then `privileged("chown")` to the invoking uid/gid (mode `0755` — conventional for a per-user base directory under `/run/media`, matching udisks2's own convention) — do this once per mount call, cheap to check-and-skip if it already exists (e.g. `base.exists()` first, avoiding an unnecessary `sudo` prompt on every unlock)
  - [x] Mount point: `base.join(tomb_name)`. Replace the current `std::env::temp_dir().join(format!("tomb-fido2-{}-{suffix}", mapper.name))` (`:898`) and its random-suffix generation — the suffix (via `random_hex_suffix()`, `:111`) is no longer needed for the primary path, only as a *fallback* (next bullet)
  - [x] Collision handling: if `std::fs::create_dir(&mountpoint)` fails with `ErrorKind::AlreadyExists`, retry once with `base.join(format!("{tomb_name}-{suffix}"))` using the existing `random_hex_suffix()` helper (shortened, e.g. 4 hex chars is plenty for a rare collision) — a bounded number of attempts (e.g. 3), returning a clear `AdapterFailure` if all are exhausted. Any other `create_dir` error (permission denied, etc.) still propagates immediately as today, unchanged
  - [x] This does **not** violate AD-12's "mount point is not stored/remembered" rule — the path is still recomputed fresh from `mapper.source_path` on every call, exactly like today; it's just a different (and now non-random, so collision-checked) formula. `close` (Story 3.1) still discovers the live mountpoint via the kernel's own mount table (`findmnt`), never by reconstructing this path — don't build a shortcut that skips that discovery
- [x] Task 4: Positional path arguments (AC: #3)
  - [x] `src/cli/main.rs`: remove `#[arg(long)]` from the `path` field on `CreateMode::File` (`:46-47`), `CreateMode::Device` (`:63-64`), and `Commands::Unlock` (`:33-34`) — in clap's derive API, a field with no `long`/`short` attribute is positional by default, so this is the entire mechanical change (no other attribute needed)
  - [x] No change to `run_create`/`run_unlock`/`domain::workflows::create::run`/`domain::workflows::unlock::run` — they already take `path`/`target` as plain values; only how clap *parses* it from argv changes
- [x] Task 5: Update fakes, unit tests, and hardware tests (AC: #1, #2, #3)
  - [x] `tests/unit/cli.rs`: `unlock_help_lists_path_flag` (`:78-82`) currently asserts the rendered help contains `"--path"` — after Task 4, clap's help output shows a positional placeholder (e.g. `<PATH>`) instead. Rename/update this test to assert the positional form instead (check clap's actual rendered output empirically rather than guessing the exact placeholder text)
  - [x] `tests/unit/fakes.rs`'s `FakeFilesystemBackend::mount` currently returns a deterministic `/tmp/fake-mount-<mapper.name>` path (Story 1.7) — this is fine to leave as-is for existing tests that only care *that* mount was called, but if any test asserts on the exact path shape, update it to reflect that path derivation is now basename-based, not purely `mapper.name`-based (the fake doesn't need to replicate the real adapter's `/run/media` logic — it's a domain-level fake, and `mount`'s real path-construction logic lives entirely in `adapters::exec`, untested by `domain`-level fakes either way)
...
- Task 2: `FilesystemBackend::mount` now calls `invoking_identity()` and `privileged("chown").arg("<uid>:<gid>")` on the mount point right after `mount` succeeds and before the existing `chmod 0700`, fixing the actual root-ownership bug. Failure branches (identity lookup, chown) reuse the exact same unmount+remove_dir+`AdapterFailure` cleanup shape as the pre-existing `chmod` failure branch. `cargo build --lib` and `cargo test --test unit` (55 passed) both green.
- Task 3: `mount` now fetches `invoking_identity()` once up front (reused for both the `/run/media/<username>` base dir and the mount-point chown from Task 2), derives `tomb_name` via `mapper.source_path.file_stem()`, lazily creates+chowns the `0755` base dir only when missing, and replaced the old `/tmp` + full random-suffix path with a new `create_mount_point(base, tomb_name)` helper that tries the plain basename first and falls back to a 4-hex-char-suffixed name on `ErrorKind::AlreadyExists`, bounded to 3 attempts. `cargo build --lib`, `cargo clippy --all-targets -- -D warnings`, `cargo fmt --check`, and `cargo test --test unit` (55 passed) all green.
- Task 4: `src/cli/main.rs` — removed `#[arg(long)]` from `path` on `CreateMode::File`, `CreateMode::Device`, and `Commands::Unlock`; clap now renders it as a positional `<PATH>` (verified empirically via `cargo run -- unlock --help` / `create file --help`). No changes needed to `run_create`/`run_unlock`/domain workflows. Bundled in the same commit: renamed `tests/unit/cli.rs`'s `unlock_help_lists_path_flag` → `unlock_help_lists_path_as_positional` (Task 5's first bullet) since leaving it unrenamed would have broken the suite — the remaining Task 5 work (fakes review, hardware test extension) is still to come. `cargo build --lib`, clippy, fmt, and `cargo test --test unit` (55 passed) all green.
- Task 5: Reviewed `tests/unit/fakes.rs`'s `FakeFilesystemBackend::mount` and `tests/unit/unlock.rs`'s assertion on its exact `/tmp/fake-mount-<mapper.name>` shape — left unchanged per Dev Notes, since no test needs the fake to replicate the real adapter's basename/`/run/media` logic. Extended `tests/hardware/main.rs`: added `id_output`/`assert_owned_by_invoking_user`/`assert_mountpoint_under_run_media` helpers, wired them into both existing unlock scenarios (file- and device-backed), and added a new `unlock_falls_back_to_a_suffixed_mount_point_on_a_basename_collision` scenario mounting two same-basename tombs to exercise the collision fallback. Fixed a `clippy::unnecessary_lazy_evaluations` hit in both the new test helper and `mount`'s own `tomb_name` derivation (`unwrap_or_else` → `unwrap_or`). `cargo build --lib`, `cargo test --test unit` (55 passed), `cargo clippy --all-targets -- -D warnings`, `cargo fmt --check`, and `cargo test --test hardware --no-run` all green.
- **Hardware verification confirmed green end-to-end**: the user ran `make test-hardware` (root, real FIDO2 key) and reported all scenarios passed — the two pre-existing Story 1.5/1.6/1.7 scenarios plus this story's new ownership/`/run/media` assertions on both existing unlock scenarios and the new basename-collision scenario. AC #1 and AC #2 are now verified on real hardware, not just via fakes/unit tests.
