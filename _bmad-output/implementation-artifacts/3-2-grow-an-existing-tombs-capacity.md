...
Both fixes are unit-tested (`tests/unit/resize.rs`, updated for the new `filesystem_size` mechanism) and pending a hardware re-run to confirm both scenarios now pass end to end.
...
- Task 9: hardware run found two real bugs, both fixed (see Dev Notes "Task 9 hardware findings"): (1) tier 2's grow-only check used the LUKS mapping's own raw size, which always reports full backing storage on reopen regardless of Story 1.6 headroom — replaced with a new `FilesystemBackend::filesystem_size` (dumpe2fs-based) that reads the actual ext4 superblock size; (2) `resize2fs` refused to grow an unmounted, never-checked filesystem — `growfs` now runs `e2fsck -f -p` first. `tests/unit/resize.rs` updated for the new mechanism (99 unit tests still pass). Hardware re-run pending to confirm both fixes.
