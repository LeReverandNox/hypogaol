...
- [ ] Task 1: Add `LuksBackend::resize` port method (AC: #1, #4)
  ...
  - [ ] Implement in `ExecAdapter`: `cryptsetup resize <name>` against the already-open mapping (subject to Task 0's spike outcome — if the keyring doesn't carry over, this implementation needs a different mechanism entirely; do not guess here).
  - [ ] Add a live "current provisioned size" query the workflow needs for AC #3's grow-only check. Reuse `FilesystemBackend::device_capacity` (already a generic `blockdev --getsize64 <path>` call — it works unmodified against `/dev/mapper/<name>` once the mapping is open, no port change needed there) rather than inventing a new method.

- [ ] Task 2: Add `LuksBackend::read_filesystem` port method (AC: #5)
  - [ ] Add `fn read_filesystem(&self, path: &Path) -> Result<Filesystem, DomainError>` to `src/ports/luks_backend.rs` — reads the `filesystem` field off the `systemd-fido2` token (written once by `create`'s `write_fido2_token_metadata`, `src/adapters/exec/mod.rs:661-744`; AD-2). Reuse the existing private `dump_json_metadata`/`tokens_object` helpers already in `src/adapters/exec/mod.rs` (used by `list_fido2_keyslots`, `enrolled_key_labels`) — do not re-implement JSON parsing.
  - [ ] Only one `Filesystem` variant exists today (`Ext4`) — parse the stored string back to the enum, returning `DomainError::AdapterFailure` for any unrecognized value (defensive; should never happen given `write_fido2_token_metadata` is the only writer).
  - [ ] This can run either before or after `luks.open` — it reads header/token state, not the live mapping, so it does not require the mapping to be open. Prefer running it early (alongside the AC #3 grow-only check) since a read failure here should abort before anything is touched.
