...
- [ ] **Task 5: Update `README.md`'s title and working-title disclaimer** (AC: #1)
  ...
  - **Explicitly out of scope for this story:** the many other body-prose mentions of `tomb-fido2` as the product's self-reference further down in `README.md` (e.g. "tomb-fido2 exists to do one job well...", "tomb-fido2 tells you exactly what to do", "tomb-fido2 refuses to remove your last remaining valid key", the "Break-glass recovery (no tomb-fido2 required)" heading, etc.) — leave every one of those untouched. Per the epics/sprint-change-proposal split, only the H1 title (+ this disclaimer) belongs to Story 5.1; the domain-noun "tomb" (the container concept) belongs to Story 5.2, and top-level tagline/copy belongs to Story 5.3. **This leaves a real gap**: body-prose self-references to the product name itself (`tomb-fido2` used as "this tool", not the container noun) aren't covered by any Epic 5 story's AC as currently scoped — flag this to the user/PM after this story lands rather than silently absorbing it here or leaving it permanently unaddressed.
...
- [ ] **Task 9: Full regression pass** (AC: #1, #2, #3)
...
- Files touched by this story: `Cargo.toml`, `Cargo.lock` (regenerated, not hand-edited), `_bmad/bmm/config.yaml`, `flake.nix`, `README.md` (title + disclaimer line only), `CHANGELOG.md` (verify-only, no edit expected). No `src/` or `tests/` files are expected to change (Task 7 is verification-only, per AD-13).
