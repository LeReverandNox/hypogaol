...
- [x] [Review][Decision] Partial-failure recovery: a retry after `umount` succeeds but `luks.close` fails can never reach `luks.close` again — `fs.umount` succeeding then `luks.close` failing leaves the dm-crypt mapping open while the filesystem is already unmounted. A retry of `close` re-runs `fs.umount` first, which now finds nothing mounted and returns "not currently mounted" before ever reaching `luks.close` — there's no CLI path to just retry the close half. **Resolved: idempotent retry.** `close::run` now treats `umount`'s "not currently mounted" outcome as already-done and proceeds to call `luks.close` anyway, so a repeat `close` call self-heals a dangling open mapping. [src/domain/workflows/close.rs:29-36]

- [x] [Review][Patch] `close`'s own `luks.close` failures are misclassified by the pre-existing generic `"cryptsetup"` bucket in `ux.rs`, before ever reaching the new close-specific branch [src/cli/ux.rs:138-159] — `ExecAdapter::close` (`src/adapters/exec/mod.rs:1016-1033`) formats failures as `"cryptsetup close failed: {stderr}"`; the generic `inner.contains("cryptsetup")` check (line 138) matches first since it's ordered before the new `umount`/`findmnt` branch (line 144), so a `close`-time `luks.close` failure gets translated to "your security key or its PIN may not have been accepted in time" — nonsensical for `close`. This is exactly the "marker bleed" bug class Task 5 and the Epic 2 retro flagged as high-risk, for a case Task 5's new branch didn't anticipate. Independently found by both the adversarial and edge-case review layers. Fixed: added a `"cryptsetup close"` marker branch ordered before the generic `cryptsetup` bucket, with a new regression test.
- [x] [Review][Patch] No unit test covers `close::run`'s `luks.close` failure path [tests/unit/close.rs] — only the happy path and an `umount` failure are tested; nothing exercises "umount succeeds, then luks.close fails," the exact path the finding above lives in. Fixed: added `luks_close_failure_after_a_successful_umount_still_propagates_as_an_error`.
- [x] [Review][Patch] `findmnt -n -o TARGET` output isn't restricted to a single line [src/adapters/exec/mod.rs, `umount`] — if the device is mounted at more than one target, the multi-line stdout is `.trim()`-ed as a whole and passed to `umount` as one argument (embedded newline and all), producing an obscure low-level failure instead of a clean translated message. Fixed: takes only the first line of `findmnt`'s output.
- [x] [Review][Patch] `umount`'s `rmdir` doesn't verify it owns the directory before deleting it [src/adapters/exec/mod.rs, `umount`] — `remove_dir` runs on whatever `findmnt` reports as the mount target, without checking it's under this tool's own `/run/media/<user>/` mount root; if the mapping were ever mounted elsewhere, a successful `close` would silently delete that unrelated, now-empty directory. Fixed: `remove_dir` is now guarded to paths under `/run/media/`.
- [x] [Review][Patch] "Not currently mounted" message conflates "already closed" with "never unlocked" [src/adapters/exec/mod.rs, src/cli/ux.rs] — `findmnt` returns nothing in both cases, so the message "This tomb doesn't look like it's currently mounted" reads oddly when the tomb was never unlocked at all (the actionable advice there is "run unlock," not "check if it's mounted"). Fixed: `umount` now checks the dm-crypt device node's existence first and returns a distinct "no active mapping" error/message for the never-unlocked case.
- [x] [Review][Patch] Stale test name/comment references the removed `todo!()` [tests/unit/workflows.rs:54] — `close_run_stops_at_preflight_before_reaching_its_own_todo` and its "which would otherwise panic" comment still refer to the `todo!()` this diff deletes; the test still passes but now documents something no longer true of the code it exercises. Fixed: renamed to `close_run_stops_at_preflight_before_touching_any_port`.

- [x] [Review][Defer] TOCTOU race between the `findmnt` check and the `umount` call [src/adapters/exec/mod.rs, `umount`] — deferred, pre-existing risk pattern shared with `mount`'s own non-atomic multi-step subprocess sequence; low probability, no clean fix without a different unmount mechanism
- [x] [Review][Defer] Silent `rmdir` failure after a successful `umount` is swallowed with no user-facing warning [src/adapters/exec/mod.rs, `umount`] — deferred, mirrors the exact same `let _ = std::fs::remove_dir(...)` pattern `mount`'s own cleanup-on-error paths already use (per this story's own Dev Notes); established precedent, not a new inconsistency
- [x] [Review][Defer] No unit test exercises the real `ExecAdapter::umount` subprocess wiring (`findmnt`/`umount`/`rmdir`) — deferred, covered only by manual-only `#[ignore]`d hardware tests, consistent with AD-7's established testing standard for every other adapter method in this codebase
- [x] [Review][Defer] Ambiguous CLI contract for `close`'s `path` argument — deferred, a user may reach for the mounted directory or `/dev/mapper/vault-*` node rather than the original backing path, but this mirrors the exact same convention already used by `unlock`/`enroll`/`revoke`, not a new ambiguity
- [x] [Review][Defer] AD-8's spec text says `FilesystemBackend`'s `mount`/`umount` take a `Filesystem` enum parameter, but neither does — deferred, pre-existing gap `mount` already had, harmless under v1's ext4-only scope, flagged by the Acceptance Auditor as non-blocking
- [x] [Review][Defer] `close` can't run at all if the backing path was deleted while the tomb is still open [src/domain/mapping_name.rs] — deferred, `mapping_name`'s `std::fs::canonicalize` requiring the path to exist is shared by every workflow using this helper (`unlock`/`revoke`/`resize`), not introduced by this story
