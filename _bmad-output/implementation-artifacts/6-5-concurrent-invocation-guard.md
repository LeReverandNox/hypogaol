...
2. **Given** any mutating workflow (`create`, `enroll`, `revoke`, `close`, `resize`), **when** it runs, **then** it acquires a non-blocking, process-scoped lock on the target as its second step, immediately after preflight passes. [Source: epics.md#Story 6.5, lines 882-884] — **Note (2026-08-10, post-review):** the mechanism actually shipped is a Linux abstract-namespace `AF_UNIX` socket, not `flock(2)` as originally worded here — see Review Findings' third finding for why (`flock` self-deadlocked against `cryptsetup`/`systemd-cryptenroll`'s own internal locking). The non-blocking, per-target, second-statement behavior this AC actually requires is unchanged.
...
- [x] **Task 12: A real, hardware-independent regression test for the actual lock behavior** (AC: #2, #5 — the only task in this story that exercises real OS-level locking, not a fake)
...
- [x] **Task 13: Full regression pass**
  - `cargo build` succeeds. `make test` passes with all prior tests green (baseline **237 total: 17 lib + 220 `tests/unit`**, verified fresh against this story's own `baseline_commit` via a real `cargo test` run — see Completion Notes' 2026-08-10 correction; Story 6.4's own claimed 235/17+218 was itself never re-verified and turned out to be off by 2) plus this story's new tests. **Every existing test that shares a `CallLog` across `create`/`enroll`/`revoke`/`close`/`close_all`/`resize`/`slam`'s fakes and asserts an exact call sequence (or `log.borrow().is_empty()`) will need `"lock_target"` inserted at the correct position** — this is the same class of ripple Story 6.4's `check_prerequisites` logging change caused (54 assertions across 9 files that time). `unlock.rs`/`info.rs` tests are unaffected (no `lock_target` call added there). Fix every compiler/assertion failure the real `cargo test` run surfaces; do not trust this task list's own file enumeration as exhaustive — same discipline Story 6.4's Dev Notes documents for its own signature-ripple.
...
- [x] [Review][Bug] `lock_target_path` fails outright on a bare relative filename (e.g. `hypogaol create file --size 64M volume.img`), surfacing as a misleading "Hypogaol couldn't find volume.img" error — `Path::parent()` on a path with no directory component returns `Some("")` (the empty path), not `None`, so the intended `unwrap_or_else(|| Path::new("."))` fallback never triggers and `canonicalize("")` fails with `ENOENT`. Found post-review (2026-08-10, reported by `LeReverandNox` while dry-running the story's own two-process hardware verification plan). Resolved as a patch: `src/domain/mapping_name.rs`'s `lock_target_path` now explicitly checks for an empty parent and falls back to `Path::new(".")` in that case too. Added a regression test in `tests/unit/mapping_name.rs` using a bare relative filename — every pre-existing `lock_target_path` test used an absolute path (`std::env::temp_dir()`), so none exercised this branch. [src/domain/mapping_name.rs, tests/unit/mapping_name.rs]
- [x] [Review][Bug][Critical] A genuine self-deadlock: `cryptsetup` takes its own internal lock on the LUKS2 container for essentially every operation, including read-only ones (`luksDump`) — reproduced directly against a real file with an externally-held `flock`, `cryptsetup luksDump`/`luksFormat` hung until killed. Once this story's own `LockGuard` is held for the whole workflow (AD-20), any subsequent `cryptsetup` subprocess call against the same target blocked forever waiting for a lock the same process would never release until that subprocess finished — a real self-deadlock within a single invocation, not a race between two. Surfaced as `hypogaol create file --size 64M volume.img` hanging indefinitely on a second run against the already-existing file: the file now exists, so `fs.lock_target` locks the file itself (not its parent), and the very next call, `luks.has_marker_token`, needed cryptsetup's own lock on that same file. Found post-review (2026-08-10, reported by `LeReverandNox`). Resolved as a patch: `--disable-locks` added to every `cryptsetup` invocation in `src/adapters/exec/mod.rs` that touches a specific LUKS2 device/mapping (`luksFormat`, `luksOpen`, `luksDump`, `isLuks`, `token export`/`import`/`remove`, `luksKillSlot`, `resize`, `status`, `close`, `open`) — including `open()` (used only by `unlock`, which never acquires this story's own lock per AC #4): leaving cryptsetup's own internal locking active there would silently reintroduce blocking behavior AC #4 explicitly excludes `unlock` from, since that internal lock and our own `flock` operate on the same underlying file. `cryptsetup`'s own locking is redundant once our own lock is the sole serialization mechanism — this story's whole premise. Verified the fix with a real, hardware-independent, no-`sudo`-needed regression test in `tests/unit/lock_target.rs`: formats a real LUKS2 container, holds a real `lock_target`, then calls the real (non-faked) `has_marker_token` on a background thread bounded by a 10s timeout — confirmed the test genuinely catches the regression by reverting the fix locally, watching it fail after the timeout, then restoring it and confirming it passes in ~0.05s. [src/adapters/exec/mod.rs, tests/unit/lock_target.rs] **Superseded by the third finding below** — the `--disable-locks` patch fixed `cryptsetup` specifically, but the next finding shows the same deadlock class recurring in a tool with no such flag, and replaces the underlying mechanism instead of patching each affected binary individually.
- [x] [Review][Patch] Document that `lock_target`'s abstract-socket mechanism is scoped to a single Linux network namespace — two invocations under different network namespaces (containers, a systemd unit with `PrivateNetwork=yes`, `unshare --net`) would each bind their own copy of the same abstract name and never contend, silently defeating AC #1 with zero error. Resolved 2026-08-10 (decided by LeReverandNox): accept as a documented limitation rather than reintroducing a disk-based lock file. Fixed: doc comments added to `bind_abstract_lock` (`src/adapters/exec/mod.rs`), `FilesystemBackend::lock_target` (`src/ports/filesystem_backend.rs`), and AD-20's Rule (`ARCHITECTURE-SPINE.md`), all stating the namespace-scoping assumption explicitly. [src/adapters/exec/mod.rs, src/ports/filesystem_backend.rs, architecture/architecture-tomb-fido2-2026-07-22/ARCHITECTURE-SPINE.md#AD-20]
- [x] [Review][Patch] `close_all`/`slam` cannot even attempt to force-close a mapping whose backing storage (source_path and its parent directory) has both vanished from disk — `lock_target_path`'s canonicalize-or-parent-fallback returns a hard `AdapterFailure`, aborting that mapping's close/slam attempt before any unmount/signal-escalation runs, regressing against slam's documented zero-configuration emergency-close purpose. Resolved 2026-08-10 (decided by LeReverandNox): add a `lock_mapping(&self, name: &str)` method deriving its lock name the same way `lock_target` does for an already-existing path (via `mapping_name::mapping_name`), so `close_all`/`slam` lock by the already-known, filesystem-independent `mapper.name` — identical lock name to a concurrent path-keyed operation against the same volume, but no canonicalize dependency. Fixed: added `FilesystemBackend::lock_mapping(name, display_path)`, backed by a shared `bind_abstract_lock` helper `lock_target` now also routes through (via `mapping_name::mapping_name(path)` when it exists, falling back to the parent-directory derivation only when it doesn't). `close_all::run`/`slam::run` now call `fs.lock_mapping(&mapper.name, &mapper.source_path)` instead of `fs.lock_target(&mapper.source_path)`. New tests: `close_all.rs`/`slam.rs` each gained a regression test proving a mapping whose backing path can't resolve still locks/closes successfully; `lock_target.rs` gained a real-`ExecAdapter` test proving `lock_mapping` and `lock_target` derive the identical lock name for the same volume (cross-workflow contention preserved) and a second proving `lock_mapping` succeeds when `display_path` doesn't exist on disk. [src/ports/filesystem_backend.rs, src/adapters/exec/mod.rs, src/domain/workflows/close_all.rs, src/domain/workflows/slam.rs, tests/unit/fakes.rs, tests/unit/close_all.rs, tests/unit/slam.rs, tests/unit/lock_target.rs]
- [x] [Review][Patch] `LockContention`'s error message echoes the caller's original path even when the parent-directory fallback means a different resource was actually locked (the fresh-create-file case) — misleading about what's actually contended. Fixed: `lock_target`'s fallback branch now reports the resolved parent-directory target as the contention error's path, not the caller's own not-yet-created file path. New test: `real_lock_target_contention_error_names_the_locked_directory_not_either_uncreated_files_own_path` (`tests/unit/lock_target.rs`), against a real `ExecAdapter`. [src/adapters/exec/mod.rs, tests/unit/lock_target.rs]
- [x] [Review][Patch] Confirmed reproducible temp-fixture-path collision: `tests/unit/close.rs` and `tests/unit/create.rs` both add `RealFixtureFile::create("lock-target-happy-path")`/`("lock-contention")` using the same un-prefixed naming convention — unlike this diff's own `enroll.rs`/`resize.rs` additions, which correctly prefix (`enroll-`/`resize-`). Two independent tests write/read/remove the identical absolute temp path, running as threads inside one shared `cargo test` process. Fixed: `create.rs`'s two fixture names prefixed to `"create-lock-target-happy-path"`/`"create-lock-contention"`. [tests/unit/close.rs:338,355, tests/unit/create.rs]
- [x] [Review][Patch] Completion Notes' self-reported baseline/new-test counts are wrong — independently verified via a worktree at `baseline_commit` (3f630db) + real `cargo test`: baseline is **17 lib + 220 tests/unit = 237 total**, not the claimed "235 (17+218)"; **+21** new `#[test]` fns added (verified via `grep -c '+#[test]'` on the diff), not the claimed "+23". Final total (258) is still correct, but the arithmetic behind it isn't (220+21=241, not 218+23). Fixed: Completion Notes and Task 13 corrected with the verified numbers and a note on the provenance (Story 6.4's own claimed final count was itself never re-verified and was off by 2). [_bmad-output/implementation-artifacts/6-5-concurrent-invocation-guard.md, Completion Notes]
- [x] [Review][Patch] AC #2's literal text ("acquires a non-blocking flock-based lock") no longer matches the shipped abstract-socket mechanism — AD-20 and the port/adapter doc comments were updated for the pivot, but the story's own AC/Task/References wording wasn't. Fixed: AC #2 reworded to "non-blocking, process-scoped lock" with a note pointing to the third Review Finding for why; Task 12's title and one stale Completion Notes label corrected. Historical narrative describing the original `flock`-based design/rationale in Tasks 2/4/5/12's bodies and Dev Notes is left intact, per this file's own convention of not rewriting superseded plan-level text. [_bmad-output/implementation-artifacts/6-5-concurrent-invocation-guard.md]
- [x] [Review][Patch] The disclosed hardware-verification gap (two-real-process `revoke` race, AC #1, noted in Completion Notes) isn't propagated to `sprint-status.yaml`'s `action_items:` list, breaking this project's own established convention for tracking exactly this kind of disclosed-but-deferred gap. Fixed: added an epic-6 `action_items` entry mirroring the existing format. [_bmad-output/implementation-artifacts/sprint-status.yaml]
- [x] [Review][Defer] Non-UTF-8 path bytes are lossy-converted (`to_string_lossy()`) before hashing into the lock name, so two distinct paths could theoretically collapse to the same lock name — deferred, pre-existing: identical pattern already shipped, unchanged, in `mapping_name::mapping_name`; not introduced by this story. [src/adapters/exec/mod.rs:2572-2574, src/domain/mapping_name.rs:45]
- [x] [Review][Defer] No automated regression test exists specific to the `systemd-cryptenroll` deadlock (only `cryptsetup`/`has_marker_token` is exercised in `tests/unit/lock_target.rs`) — deferred: the abstract-socket mechanism structurally can't touch the target file at all anymore, so the existing test already proves the general "our lock never conflicts with any subprocess call against the target" property; a dedicated test would be corroborating, not load-bearing, and needs real `systemd-cryptenroll`/hardware to be meaningful. [tests/unit/lock_target.rs]

- [x] [Review][Bug][Critical] The second finding's `--disable-locks` patch only fixed `cryptsetup`; `systemd-cryptenroll` (used by `enroll_fido2_key`, called by both `enroll` and `create`'s bootstrap-enroll step) is built directly on libcryptsetup and takes the exact same kind of internal LUKS2 container lock, but exposes **no `--disable-locks` equivalent on its CLI, and no environment-variable override** (confirmed by inspecting `--help` and the binary's strings). Surfaced as `hypogaol enroll --label second-key <path>` freezing silently right after the CLI's own interactive FIDO2-device-selection prompts, before `systemd-cryptenroll`'s own touch-prompt output ever appears — confirmed live: with an external lock held on the target file, `systemd-cryptenroll --fido2-device=<device> --unlock-key-file=... <path>` produced zero output before timing out; without any lock held, the same command immediately printed `Initializing FIDO2 credential on security token.` Found post-review (2026-08-10, reported by `LeReverandNox`). Since no per-invocation flag/env-var fix exists for `systemd-cryptenroll`, and the user explicitly asked for a memory-only alternative rather than a sibling lock file on disk, resolved by replacing `lock_target`'s entire mechanism: a Linux abstract-namespace `AF_UNIX` socket bound to a name derived by hashing the target's canonicalized path (reusing `mapping_name`'s FNV-1a hash, promoted to `pub(crate)`) — a kernel-only resource, never written to any filesystem, that can never collide with any current or future `cryptsetup`/`systemd-cryptenroll` internal locking regardless of which operation triggers it. Every `--disable-locks` addition from the second finding was reverted (no longer needed, and reverting restores `cryptsetup`'s own protection against a genuinely-concurrent non-hypogaol `cryptsetup` invocation, which this tool's own lock never covered anyway); `libc` removed as a direct dependency (no longer used anywhere, still transitive via `getrandom`, unchanged in `Cargo.lock`). `LockGuard`'s shape, the trait signature, `DomainError::LockContention`, and every workflow's wiring/every existing test needed zero changes — the port abstraction already drew the boundary in exactly the right place; this pivot is confined to `ExecAdapter`'s own implementation. Verified live against real `systemd-cryptenroll` a second time, post-fix: binding the exact abstract-socket name the real code derives, then running `systemd-cryptenroll` against the same volume, reaches its normal touch-prompt output immediately. AD-20 in `ARCHITECTURE-SPINE.md` updated to describe the new mechanism directly (not layered as an "amended" annotation, since the `flock`-based rule was never actually shipped — this story's PR hadn't merged). [src/adapters/exec/mod.rs, src/domain/mapping_name.rs, src/domain/types.rs, src/ports/filesystem_backend.rs, tests/unit/lock_target.rs, Cargo.toml, architecture/architecture-tomb-fido2-2026-07-22/ARCHITECTURE-SPINE.md]
...
- **Correction (2026-08-10, code review):** the baseline/new-test counts below were originally reported as "235 total (17 lib + 218 tests/unit)" baseline and "+23 new tests", matching Story 6.4's own Completion Notes claim — but Story 6.4's own claim was itself never re-verified against real output and turned out to be wrong. Re-checked directly by checking out `baseline_commit` (3f630db) into a worktree and running `cargo test --lib`/`--test unit`: the real baseline is **237 total (17 lib + 220 tests/unit)**, not 235 (17+218). Counting actual new `#[test]` functions added in the diff (`grep -cF '+#[test]'`) gives **21**, not 23 — the original per-file breakdown below actually sums to 21 (2+2+2+2+3+2+2+2+4), so the "+23" headline figure was simply arithmetic error on top of the wrong baseline, not a second independent discrepancy. The final total, **258 total (17 lib + 241 tests/unit)**, is still correct (237+21=258), by coincidence of the same final number both ways (218+23=241 and 220+21=241). Per-file breakdown, corrected attribution: `create.rs` +2, `enroll.rs` +2, `revoke.rs` +2, `close.rs` +2, `resize.rs` +3, `close_all.rs` +2, `slam.rs` +2, `lock_target.rs` +2 (new file, Task 12), `mapping_name.rs` +4 (2 direct `lock_target_path` unit tests per the Dev Notes watchlist item, plus 2 regression tests tied to the first Review Finding's bare-relative-filename/parent-also-missing fix — not "3 from Task 12", which is specifically `lock_target.rs`, a separate file).
- Second post-review finding (2026-08-10, also reported by `LeReverandNox`): `cryptsetup` itself takes an internal lock on the LUKS2 container for nearly every operation, which self-deadlocked against this story's own `LockGuard` once held for a whole workflow — see Review Findings above for the full mechanism and fix (`--disable-locks` everywhere). This is more architecturally significant than the first finding: it isn't a bug in this story's own new code so much as an unanticipated interaction between this story's design (hold one process-level lock for the whole workflow) and `cryptsetup`'s own, previously-unknown internal locking behavior. Confirmed via direct reproduction against a real file (not inferred): an external `flock` made real `cryptsetup luksDump`/`luksFormat` hang, and `--disable-locks` against the same held lock returned immediately. **Superseded by the third finding below.**
- Third post-review finding (2026-08-10, also reported by `LeReverandNox`, while testing the second finding's own fix against real `enroll`): the same deadlock class recurred in `systemd-cryptenroll`, which has no `--disable-locks`-equivalent flag or environment-variable override — see Review Findings above. Rather than continue patching each affected binary individually (an open-ended list with no way to prove completeness short of exercising every call site against real hardware, which is exactly how both this finding and the second one were caught), replaced `lock_target`'s entire mechanism with a Linux abstract-namespace socket — a kernel-only resource structurally incapable of colliding with any subprocess's own file/device locking, closing this bug class permanently rather than case-by-case. The user explicitly requested a memory-only alternative over a sibling lock file on disk once a design change was already on the table. Every `--disable-locks` addition from the second finding was reverted (superseded, no longer needed) and `libc` removed as a direct dependency (no longer used). Verified live twice: the original `enroll` freeze reproduced against real `systemd-cryptenroll` with an externally-held lock (zero output before timeout, versus immediate `Initializing FIDO2 credential...` output with no lock held), then re-verified fixed by binding the exact same abstract-socket name the real code derives and confirming `systemd-cryptenroll` reaches that same output immediately regardless.
...
- Task 12's real, hardware-independent lock regression test (`tests/unit/lock_target.rs`) passes: constructs a real `ExecAdapter`, locks a real temp file, proves a second `lock_target` call on the same path while the first `LockGuard` is still held returns `DomainError::LockContention`, then proves dropping the first guard releases the lock (a third call then succeeds) — the primary correctness mechanism for this story, needing no hardware.
- Hardware verification: `cryptsetup`/`fido2-token`/`systemd-cryptenroll` and `/dev/hidraw*` devices are present in this environment, but this automated session has no interactive `sudo` prompt and cannot perform a physical FIDO2 key touch — consistent with this project's standing convention (Stories 4.3, 5.1, 5.2, 6.1, 6.2, 6.3, 6.4), the literal two-real-OS-process scenario in Task 13's last paragraph (two concurrent `revoke` invocations against a real volume with 2 enrolled keys) was not run in this session. Flagged as a retrospective action item for `LeReverandNox` (`sprint-status.yaml`). **RESOLVED 2026-08-10 (LeReverandNox):** re-ran `make test-hardware` and a concurrent two-process `revoke` script against a real volume with 2 valid FIDO2 keyslots — exactly one succeeded, the other reported the lock-contention message (AC #1 confirmed on real hardware). Also directly verified the `lock_mapping`/orphaned-backing-storage fix (this review round's second decision-needed finding): deleted a mapping's backing file while it was still open, then ran `close-all`/`slam` — both closed it successfully instead of hard-failing to lock. No outstanding hardware action items remain for this story.

### File List

**Production:**
- `Cargo.toml` — `libc = "0.2"` promoted to a direct dependency, then reverted (no longer needed once `lock_target` stopped using `flock`)
- `src/domain/errors.rs` — `DomainError::LockContention(PathBuf)` variant
- `src/domain/types.rs` — `LockGuard` struct; doc comment updated for the abstract-socket mechanism
- `src/domain/mapping_name.rs` — `lock_target_path` function; `fnv1a_hash` promoted to `pub(crate)` for reuse by `adapters::exec`'s lock name
- `src/ports/filesystem_backend.rs` — `lock_target` trait method; doc comment updated for the abstract-socket mechanism
- `src/adapters/exec/mod.rs` — `lock_target` impl: originally real `flock(2)` (superseded), then `--disable-locks` added everywhere (superseded), now binds a Linux abstract-namespace `AF_UNIX` socket; `OpenOptionsExt` kept at top-level (still needed by `set_backing_file_size`/`TempKeyFile`), `AsRawFd`/`OwnedFd` import removed (no longer directly referenced)
...
- `tests/unit/lock_target.rs` — new file, Task 12's real-`ExecAdapter` lock regression test, plus a second regression test for the cryptsetup-internal-locking deadlock (both post-review findings); doc comments updated for the abstract-socket mechanism
...
**Planning artifacts:**
- `architecture/architecture-tomb-fido2-2026-07-22/ARCHITECTURE-SPINE.md` — AD-20's Rule updated to describe the abstract-socket mechanism (superseding its original `flock(2)` text, never actually shipped)
