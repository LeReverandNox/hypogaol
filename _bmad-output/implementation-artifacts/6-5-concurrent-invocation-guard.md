...
- Baseline re-verified fresh against this story's own `baseline_commit` (3f630db) via `make test`: **235 total (17 lib + 218 tests/unit)** — confirmed, not from memory, matching Story 6.4's own Completion Notes claim exactly. Final count after this story: **257 total (17 lib + 240 tests/unit)**, +22 new tests (`create.rs` +2, `enroll.rs` +2, `revoke.rs` +2, `close.rs` +2, `resize.rs` +3, `close_all.rs` +2, `slam.rs` +2, `lock_target.rs` +1 new file, `mapping_name.rs` +4 — 3 from Task 12 plus 1 more from the post-review bug fix above).
