...
- [x] [Review][Patch] `DeviceTooSmall`'s generic message omits the actual required minimum, and the Btrfs resize-floor check reports the raw requested size while the check itself compares against the post-header payload size — `src/domain/workflows/resize.rs:265-269`'s Btrfs floor rejection sets `size: new_size` (raw/whole-file bytes), but the gate it just failed is `new_size_as_payload < MIN_BTRFS_RESIZE_PAYLOAD_BYTES` (post-header payload bytes). `src/cli/ux.rs`'s `DeviceTooSmall` message ("`{path}` would only have `{size}` bytes for a volume — that's too small to be usable.") never states which minimum applies, for any of the three floors this story touches (generic 32 MiB, XFS 350 MiB create-time, Btrfs 260 MiB resize-time). Resolved as a patch (2026-08-09, decided by LeReverandNox): report the correct basis and state the actual minimum. Fixed: added a `minimum: u64` field to `DeviceTooSmall`; `resize.rs`'s Btrfs check now reports `new_size_as_payload` (matching the basis it actually compared), `create.rs`'s two call sites pass `size_floor_for(filesystem)`; `ux.rs`'s message now states the minimum. [src/domain/workflows/resize.rs:265-269, src/cli/ux.rs, src/domain/errors.rs, src/domain/workflows/create.rs]

- [x] [Review][Patch] `with_transient_mount` creates its scratch mountpoint via an unprivileged `std::fs::create_dir_all` directly under root-owned `/run` [src/adapters/exec/mod.rs:1609-1615] — Fixed: mountpoint creation/removal now goes through `privileged("mkdir"/"rmdir")`, mirroring `mount()`'s own `/run/media/<user>` precedent.
- [x] [Review][Patch] Marker-bleed reintroduced for `with_transient_mount`'s umount ("conclude")-failure path — the message text doesn't contain the substring `ux::translate`'s new branch checks for, and even if it did, the pre-existing `"umount"/"findmnt"` bucket is checked first and matches real `umount(8)` stderr [src/adapters/exec/mod.rs:1646-1660, src/cli/ux.rs] — Fixed: both messages now carry a dedicated `"hypogaol-transient-mount"` marker, and the `ux::translate` branch checking for it was moved ahead of every bucket that does a bare `"mount"/"umount"/"findmnt"` substring match. Two dedicated regression tests added (prepare-failure and conclude-failure paths).
- [x] [Review][Patch] `with_transient_mount`'s fixed, deterministic mountpoint plus an unguarded `remove_dir` allows a stale/duplicate mount to be silently mounted-over within a single resize invocation [src/adapters/exec/mod.rs:1609-1661] — Fixed: mountpoint name now carries a fresh random suffix per call, mirroring `create_mount_point`'s own collision-avoidance discipline.
- [x] [Review][Patch] `FakeFilesystemBackend::check_prerequisites` ignores its `filesystem` argument when deciding pass/fail, so no unit test can prove resize's second, type-specific preflight call is load-bearing on its own [tests/unit/fakes.rs] — Fixed: added `with_failure_for_filesystem`, plus a new test (`resize_aborts_when_only_the_second_type_specific_preflight_call_finds_a_missing_toolchain`) proving the second call alone is load-bearing.
- [x] [Review][Patch] Inconsistent assertion rigor: `close_all.rs`/`slam.rs` use a weaker negative-contains check for the preflight short-circuit while `enroll.rs`/`info.rs`/`revoke.rs` use an exact `vec` assertion [tests/unit/close_all.rs, tests/unit/slam.rs] — Fixed: both tightened to the exact-`vec` form.
- [x] [Review][Patch] `FilesystemBackend` trait doc comments for `mkfs`/`growfs`/`filesystem_size` still say "v1: `Filesystem::Ext4` only" despite this story making all three Xfs/Btrfs-aware [src/ports/filesystem_backend.rs] — Fixed: doc comments updated to name all three variants.

- [x] [Review][Defer] `with_transient_mount` performs two independent, unguarded mount/unmount cycles per resize (`filesystem_size` then `growfs`) with no protection against filesystem state changing between them [src/adapters/exec/mod.rs] — deferred, pre-existing: Story 6.5 "concurrent-invocation-guard" is the planned fix for this exact class of issue
- [x] [Review][Defer] No real-hardware XFS resize scenario was exercised near the new 300MB create-time floor boundary — both hardware XFS scenarios use volumes hundreds of MiB above it [tests/hardware/main.rs] — deferred, pre-existing convention: hardware unavailable in this review session
- [x] [Review][Defer] `MIN_BTRFS_RESIZE_PAYLOAD_BYTES`'s margin above the real kernel floor (260 MiB vs. 256 MiB, ~1.5%) is thin and mostly earmarked for block-flooring elsewhere, not independently verified right at the boundary on real hardware [src/domain/workflows/resize.rs:33] — deferred, pre-existing convention: hardware unavailable in this review session
...
- Hardware verification: this session itself could not run `make test-hardware` (no interactive sudo, no physical FIDO2 touch), but `LeReverandNox` ran the 5 new scenarios directly afterward and found two real issues, both now fixed and re-verified via `cargo build`/`make test` (not yet re-confirmed on hardware — see below):
  ...
  - `LeReverandNox` re-ran all 5 hardware scenarios after both fixes (XFS size bump, Btrfs `1:max` revert + resize-floor gate below) — **all 5 pass.** Hardware verification for this story is complete; no outstanding action item.
