...
- `parse_uv_capable` (and its `parse_client_pin_configured` template) mis-parses as "not capable" on non-conformant `options:` formatting: leading whitespace before the `options: ` prefix, a separator other than exactly `", "` (e.g. CRLF line endings), or multiple `options:` lines in the output (only the first is read) — identical pre-existing technique mirrored intentionally per this story's own Task 1 instruction, not introduced by this diff. [src/adapters/exec/mod.rs:1017-1023]
- `fido2_token_supports_uv` duplicates `fido2_token_has_pin`'s `Command`/error-handling body verbatim, differing only in which parser is called on the last line — no shared helper extracted; deliberate per this story's own Dev Notes (mirrors AD-21's "reuse over new port method" precedent), and extracting a shared helper would touch `fido2_token_has_pin`, outside this story's declared scope. [src/adapters/exec/mod.rs:952-991]
- Both I/O wrappers produce the exact same error string shape (`"fido2-token -I failed for {path}: {stderr}"`), so once Story 7.5 calls both for the same device, a failure gives no way to tell which check failed from the message text alone — no call site exists yet in this diff to fix concretely; flag for 7.5's design. [src/adapters/exec/mod.rs:958-961, 984-987]
- `fido2_token_supports_uv` ships `#[allow(dead_code)]` with zero consumers, depending on Story 7.5 (still `backlog`, no committed design) to add the only planned call site, in an epic that withdrew its immediately preceding story (7.3) this same day — not actionable until 7.5's shape is known. [src/adapters/exec/mod.rs:978]
- `REAL_INFO_OUTPUT_WITH_PIN` (a single TOKEN2 capture) is the only real-hardware ground truth behind both `parse_client_pin_configured` and this story's new `parse_uv_capable`; multi-line-wrapped `options:` output from other authenticator models/firmware is untested — pre-existing limitation inherited unchanged. [src/adapters/exec/mod.rs:3551]
