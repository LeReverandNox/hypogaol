...
- [x] **Task 1: `LuksBackend::list_open_mappings()` port method (AC #1)**
  - Add to `src/ports/luks_backend.rs`: `fn list_open_mappings(&self) -> Result<Vec<MapperHandle>, DomainError>;` with a doc comment stating this is AD-17's live-discovery method — never a registry — and that it recovers each mapping's original `source_path` from `cryptsetup status`'s reported device, not a raw `/dev/loopN` node (see Dev Notes: cryptsetup(8) confirms `status` reports the loop *backing file*, not the loop device itself, for a file-backed mapping). **Corrected post-hardware-verification (2026-07-28, code-review patch):** this original understanding was wrong — see Dev Notes' "Correction from hardware verification" below. The doc comment (and this task's own text above) described the disproven `device:`-only theory; the shipped/current doc comment now states the actual `loop:`-preferred, `device:`-fallback behavior.
...
- [x] **Task 2: Implement `list_open_mappings` in `ExecAdapter` (AC #1)**
  ...
  - For each surviving name, run `cryptsetup status <name>`, find the line whose trimmed key (text before the first `:`) is `device`, and use its trimmed value as `source_path`. **Corrected post-hardware-verification (2026-07-28, code-review patch):** the shipped implementation instead prefers the `loop:` key (falling back to `device:` only when `loop:` is absent) — see Dev Notes' "Correction from hardware verification" below; the paragraph above described the disproven `device:`-only theory this task was originally written against. A `cryptsetup status` failure for one discovered name (e.g. a race where it closed between the `dmsetup ls` and this call) propagates as this whole call's `Err` — do not silently skip it; this codebase already accepts this narrow TOCTOU window as a deferred, low-likelihood risk (ARCHITECTURE-SPINE.md's Deferred section), not something this story needs to newly handle per-entry.
...
- [x] [Review][Decision] `dmsetup` was added to the shared, undifferentiated `LuksBackend::check_prerequisites` (`src/adapters/exec/mod.rs:788-790`), which `domain::preflight::check` runs as the first statement of *every* workflow (AD-4) — `create`, `unlock`, `enroll`, `resize`, `revoke`, and `info` now all hard-fail preflight if `dmsetup`/`lvm2` is missing, even though none of them call `list_open_mappings`. **Resolved (LeReverandNox, 2026-07-28): accept as-is.** This tool is only ever run inside the Nix devShell, where `lvm2`/`dmsetup` is already guaranteed present alongside `cryptsetup` — no workflow can hit this gap in practice. No code change made.
- [x] [Review][Patch] `LuksBackend::list_open_mappings`'s trait doc comment [src/ports/luks_backend.rs:67-76] still asserts `source_path` is recovered from `cryptsetup status`'s `device:` line — this is the exact claim the story's own hardware verification disproved (`device:` is the opaque `/dev/loopN` node; the real backing file is on the separate `loop:` line). The comment was never updated after the fix, and now contradicts both the Dev Notes correction and the shipped `ExecAdapter` implementation, which correctly prefers `loop:`. **Fixed:** doc comment rewritten to state the `loop:`-preferred, `device:`-fallback behavior.
- [x] [Review][Patch] The story file's own Completion Notes List (AC #1 bullet) and Task 1/Task 2 text [this file, lines ~27-36, ~183] still describe the pre-fix `device:`-only approach, even though the Dev Notes "Correction from hardware verification" section two sections below documents that this was wrong and fixed. Stale/contradictory documentation within the same story file. **Fixed:** added inline correction notes to Tasks 1/2 (preserving the original task text as historical record) and rewrote the Completion Notes AC #1 bullet to describe the actual, corrected behavior.
- [x] [Review][Patch] Zero unit-test coverage of the actual `loop:`/`device:` field-parsing logic [src/adapters/exec/mod.rs:1217, the nested `field()` fn] — this is exactly the logic that shipped a bug caught only via manual hardware testing, and it has no regression protection (`field()` is nested inside `list_open_mappings` and unreachable from the file's own `mod tests`; all `tests/unit/close_all.rs` coverage goes through `FakeLuksBackend`, bypassing this parsing entirely). Extract `field()` to module scope and add literal-fixture tests (file-backed with `loop:`, device-backed without `loop:`, missing both). **Fixed:** extracted to module-scope `cryptsetup_status_field`, added 3 unit tests in `src/adapters/exec/mod.rs`'s `mod tests` (loop-preferred, device-fallback, neither-present).
- [x] [Review][Patch] The error raised when neither field is found [src/adapters/exec/mod.rs:1229] still says `"missing a parsable device: line"`, even though `loop:` is checked first — misleading for anyone debugging a real failure. **Fixed:** message now reads "missing a parsable loop: or device: line".
- [x] [Review][Patch] `run_close_all` [src/cli/main.rs:558] unconditionally prints `"Closing every open tomb."` before checking whether any tombs are open, so the empty case prints two contradictory-sounding lines back to back: `"Closing every open tomb."` immediately followed by `"No tombs are currently open."`. **Fixed:** reworded the pre-run intro to `"Checking for open tombs to close."` (state-neutral, doesn't presuppose any exist) rather than reordering the print after `close_all::run`'s hook-warning callbacks fire — reordering would have flipped the existing intro-before-hook-warnings convention `run_close` also follows.
- [x] [Review][Defer] `list_open_mappings`'s `dmsetup ls` → `cryptsetup status` TOCTOU window [src/adapters/exec/mod.rs:1199] hard-fails the *entire* discovery call if one mapping closes mid-scan, rather than skipping just that entry — deferred, pre-existing category of accepted single-user-tool race risk. This is a deliberate, documented Dev Notes decision; the ARCHITECTURE-SPINE.md Deferred-section citation doesn't literally name this specific race, but the same "low-likelihood for a single-user cold-storage tool" reasoning it states for other concurrent-invocation races applies equally here.
- [x] [Review][Defer] No `LC_ALL=C` (or equivalent) is pinned on the `dmsetup ls`/`cryptsetup status` subprocess calls [src/adapters/exec/mod.rs:1164, 1191] whose textual output is parsed for field values for the first time in this codebase — a non-English locale could in principle break the `loop:`/`device:` field-name match. Deferred as low-likelihood for this tool's single-user, English-locale environment; no existing codebase precedent either way.
...
- AC #1: `LuksBackend::list_open_mappings` (port) + `ExecAdapter` impl added — `dmsetup ls` (run via `privileged()`, consistent with every other live device-mapper query in this adapter) filtered to `vault-`-prefixed names, then `cryptsetup status <name>`'s `loop:` line (falling back to `device:` when absent, e.g. for device-backed tombs) recovers each mapping's `source_path` — corrected per the hardware-verified fix below; `device:` alone reports the opaque `/dev/loopN` node, not the original backing path. A `cryptsetup status` failure for one name propagates as `list_open_mappings`'s own `Err`, per Dev Notes' "hard stop, never a per-entry skip" resolution.
