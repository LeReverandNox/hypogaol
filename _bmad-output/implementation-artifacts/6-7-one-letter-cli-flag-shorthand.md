...
- [x] **Task 1: Design and record the full flag → short-alias table before touching any code** (AC: #1, #2, #3, #4)
  ...
  - **Proposed table (apply exactly, or record and justify any deviation in Completion Notes):**
    ...
    | | `--unlock-fido2-device` | `-n` | ~~`-u`, first declared~~ — superseded post-implementation (LeReverandNox, 2026-08-11): `-u` reassigned to `--user-verification` for cross-subcommand consistency with `create file`/`create device`'s `-u`. Falls back to 2nd letter of "u**n**lock", same style as `scaffold_hooks`'s `-c` fallback |
    | | `--user-verification` | `-u` | ~~`-v`, collision fallback~~ — superseded post-implementation: reassigned to `-u` (first letter) so this flag's short alias is consistent across `enroll` and `create file`/`create device`, at the cost of `--unlock-fido2-device` losing the `-u` it would otherwise have claimed first |
  ...
  - **`-h`/`-V` (help/version) are clap-automatic and already excluded from every collision above** — none of the applied short letters is `h` or `V`. Do not pass `short = 'h'`/`short = 'V'` to any `#[arg(...)]`.
...
- [x] **Task 7: Add short-vs-long equivalence tests to `tests/unit/cli.rs`** (AC: #2, #3)
  - For a representative flag in each subcommand (at minimum one per subcommand that has flags — no need to test every single flag exhaustively if the parsing mechanism is proven generic, but cover at least one boolean flag and one value-taking flag per subcommand, plus every flag directly involved in a Task 1 collision), assert that parsing the same command line with the short form and with the long form produces identical `format!("{cli:?}")` output. Pattern:
    ...
    ```
  - **Explicit collision-pair coverage required** (these are the tests that actually prove AC #3, not just AC #2): for `create file`/`create device`, a test asserting `-s` maps to `--size` and *not* `--scaffold-hooks` (e.g. parse `create file <path> -s 64M` and confirm it's equivalent to `--size 64M`, while `create file <path> -c` behaves like `--scaffold-hooks`, not `--size`); similarly `-f` → `--filesystem` (not `--fido2-device`) and `-d` → `--fido2-device` (not `--filesystem`) on both `create` variants; for `enroll`, `-n` → `--unlock-fido2-device` (not `--user-verification`) and `-u` → `--user-verification` (not `--unlock-fido2-device`).
...
- Post-review tweak (LeReverandNox, 2026-08-11, PR #72 still open): `enroll`'s `--user-verification` reassigned from `-v` to `-u`, for cross-subcommand consistency with `create file`/`create device`'s `--user-verification` (`-u`) — this was a deliberate table refinement, the same kind of expected follow-up Task 1's Dev Notes call out (mirroring Story 6.6's own post-review wording iterations). Since `-u` was already claimed by `enroll`'s `--unlock-fido2-device` (first-declared-wins under Task 1's original rule), that flag now falls back to `-n` (2nd letter of "u**n**lock", same fallback style as `--scaffold-hooks`'s `-c`). Updated: `src/cli/main.rs` (2 `short` values swapped), Task 1's table (struck through with a dated note, not silently rewritten), and the 3 affected tests in `tests/unit/cli.rs` (`enroll_short_u_is_unlock_fido2_device_not_user_verification` → `enroll_short_n_is_unlock_fido2_device_not_user_verification`, `enroll_short_v_is_user_verification_not_unlock_fido2_device` → `enroll_short_u_is_user_verification_not_unlock_fido2_device`, `enroll_help_shows_short_aliases`). Re-verified: `cargo build --tests` clean, `make test` 307/307 unchanged, `cargo fmt --check`/`cargo clippy --all-targets` clean (still 5 pre-existing warnings).
...
- 2026-08-11 (post-review tweak, PR #72 still open): reassigned `enroll --user-verification` from `-v` to `-u`, per `LeReverandNox`'s live-review feedback, for consistency with `create file`/`create device`'s `-u`. `--unlock-fido2-device` (which held `-u` first) falls back to `-n`. See Completion Notes for the full before/after and re-verification.
