...
- [ ] **Task 1: Add `has_marker_token` and `remove_marker_token` to the `LuksBackend` port** (AC: #1, #2, #3, #4)
...
- [ ] **Task 3: Implement `has_marker_token` and `remove_marker_token` in `ExecAdapter`** (AC: #1, #2, #3, #4)
...
- [ ] **Task 4: Wire marker-verified resume into `domain::workflows::create::run`'s File branch** (AC: #1, #3)
...
- [ ] **Task 5: Wire marker-verified resume into the Device branch** (AC: #2, #3)
...
- [ ] **Task 6: Final-cleanup ordering — marker token removed before the bootstrap keyslot** (AC: #4)
...
- [ ] **Task 7: Confirm AC #5 (FIDO2-before-mkfs ordering) needs no code change, only a regression guard** (AC: #5)
